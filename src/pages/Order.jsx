...
export default function Order() {
  ...
  const [info, setInfo] = useState(() => {
    ...
    return saved || {
      branch: '', name: '', phone: '', battalion: '',
      paymentMethod: 'cashapp', paymentHandle: '',
    }
  })
  ...
  const add    = (id) => { if (selected.length < maxQty) setSelected(s => [...s, id]) }
  const remove = (id) => {
    const idx = selected.lastIndexOf(id)
    if (idx !== -1) setSelected(s => s.filter((_, i) => i !== idx))
  }

  // Save info to localStorage whenever it changes
  useEffect(() => {
  ...
  const Dot = ({ n }) => (
    <div style={{
      ...
      fontSize: 12, fontWeight: 700, color: step >= n ? '#fff' : '#B07040',
      transition: 'all 0.3s',
    }}>{step > n ? '✓' : n}</div>
  )
  const Line = ({ n }) => (
            ...
            {[
              ...
              >
                ...
                <div style={{ padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontFamily: "'Lato', sans-serif", fontSize: 13, color: '#7A3A10', lineHeight: 1.6 }}>
                    {cutoffText}
                  </div>
                  <div style={{
                    background: accent, color: '#fff', fontSize: 18,
      ...
      {step === 1 && (
        <div>
          ...
          <p style={{ fontFamily: "'Lato', sans-serif", fontSize: 13, color: '#B07040', marginBottom: 22 }}>
            {orderType === 'tray' ? 'Select 1 tray per order.' : 'Select up to 3 plates.'}
          </p>
              ...
              {catItems.map(item => (
                <div key={item.id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontFamily: "'Lato', sans-serif", fontSize: 11, color: '#B07040', marginTop: 2 }}>
          ...
          {selected.length > 0 && (
            <div style={{
              ...
              <span style={{ fontFamily: "'Lato', sans-serif", fontSize: 14, color: '#7A3A10' }}>
                {selected.length}/{maxQty} selected
              </span>
      ...
      {step === 2 && (
        <div>
          ...
          <div style={{ marginTop: 28, marginBottom: 8 }}>
            ...
            }}>How will you pay?</div>

            <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
              {['cashapp', 'zelle'].map(method => (
                <div key={method}
                  onClick={() => setInfo(i => ({ ...i, paymentMethod: method }))}
                  style={{
            ...
            <div className="field-group">
              <label className="field-label">
                Your {info.paymentMethod === 'cashapp' ? 'Cash App' : 'Zelle'} name / username *
              </label>
              <input
              ...
              <p style={{ fontFamily: "'Lato', sans-serif", fontSize: 11, color: '#B07040', marginTop: 6, lineHeight: 1.5 }}>
                This must match the name you use to send the payment so Obaa Yaa can confirm it's you.
              </p>
            </div>
          </div>
        </div>
      ...
      {step === 3 && (
        <div>
          ...
          <div style={{ background: 'rgba(212,84,26,0.04)', border: '1px solid rgba(212,84,26,0.12)', borderRadius: 14, padding: 16, marginBottom: 14 }}>
            ...
            <div style={{ fontFamily: "'Lato', sans-serif", fontSize: 14, color: '#1E0E04', lineHeight: 2.0 }}>
              <div>{info.name}</div>
          ...
          <div style={{ background: 'rgba(212,84,26,0.06)', border: '1px solid rgba(212,84,26,0.15)', borderRadius: 12, padding: '14px 16px', marginBottom: 8 }}>
            <p style={{ fontFamily: "'Lato', sans-serif", fontSize: 13, color: '#7A3A10', lineHeight: 1.7, margin: 0 }}>
              ...
              Send your payment of <strong>${total}</strong> via {info.paymentMethod === 'cashapp' ? 'Cash App' : 'Zelle'} to confirm.
              She will approve your order once payment is received.
            </p>
          </div>
        </div>
      )}
